<!--
---
title: Modeling with TLA$$^+$$ for Engineers
title-meta: Modeling with TLA+ for Engineers
author: jskri@proton.me — github.com/jskri/modeling-with-tla
subtitle: version 0.6
date: May 2026
abstract: |
    This tutorial introduces TLA$$^+$$ as a formal modeling tool for engineers,
    structured around three core activities: modeling and verification,
    compositional design, and implementation conformance. The first part
    establishes foundational concepts (model checking, invariants, and refinement)
    using a wall clock example, then introduces the TLA$$^+$$ prover to address
    the limitations of automated checking alone. The second part covers system
    decomposition, demonstrating how a versioned theme download system can be
    split into subcomponents, formally composed in TLA$$^+$$, and progressively
    refined from an atomic interaction model to a request-reply protocol, with
    invariants and refinement proven at each level. The third part bridges formal
    models and executable code: a C++20/ZeroMQ implementation is validated against
    the final model by extracting execution traces and replaying them through the
    checker, with a concrete bug discovery illustrating the technique's practical
    value. The overarching methodology is correct-by-construction. Each successive
    model is formally proven to implement the previous one, culminating in a
    low-level model from which the implementation is derived and its conformance
    verified per execution.
